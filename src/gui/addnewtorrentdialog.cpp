...
void AddNewTorrentDialog::setupTreeview()
{
    ...
    connect(m_contentModel, &TorrentContentModel::filteredFilesChanged, this, &AddNewTorrentDialog::updateDiskSpaceLabel);
    ...
    const auto contentHandler = new TorrentContentAdaptor(m_torrentInfo, m_torrentParams.filePaths, m_torrentParams.filePriorities);
    const auto contentLayout = static_cast<BitTorrent::TorrentContentLayout>(m_ui->contentLayoutComboBox->currentIndex());
...
void AddNewTorrentDialog::handleFilterTextChanged(const QString &filter)
{
    ...
    if (filter.isEmpty())
    {
        ...
        m_ui->contentTreeView->expand(m_ui->contentTreeView->model()->index(0, 0));
    }
    else
    {
