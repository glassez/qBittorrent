...
const QString DATA_FILE_EXTENSION = u".ini"_qs;

GenericDataStorage::GenericDataStorage(const QString &baseName, QObject *parent)
    : KeyValueDataStorage(parent)
    , m_baseName {baseName}
{
    readNativeSettings();
    ...
    connect(&m_timer, &QTimer::timeout, this, &GenericDataStorage::flush);
}
...
QVariant GenericDataStorage::loadValueImpl(const QString &key, const QVariant &defaultValue) const
{
    const QReadLocker locker {&m_lock};
    return m_data.value(key, defaultValue);
}

void GenericDataStorage::storeValueImpl(const QString &key, const QVariant &value)
{
...
void GenericDataStorage::readNativeSettings()
{
    ...
    const auto deserialize = [](QVariantHash &data, const QString &baseName) -> Path
    {
        const auto nativeSettings = std::make_unique<QSettings>((baseName + DATA_FILE_EXTENSION), QSettings::IniFormat);
    ...
    const Path newPath = deserialize(m_data, (m_baseName + u"_new"));
    if (!newPath.isEmpty())
    {
        ...
        LogMsg(tr("Detected unclean program exit. Using fallback file to restore data: %1")
    ...
    {
        deserialize(m_data, m_baseName);
    }
}

bool GenericDataStorage::writeNativeSettings() const
{
    auto nativeSettings = std::make_unique<QSettings>((m_baseName + u"_new" + DATA_FILE_EXTENSION), QSettings::IniFormat);
    ...
    case QSettings::AccessError:
        LogMsg(tr("An access error occurred while trying to write the data file."), Log::CRITICAL);
        break;
    case QSettings::FormatError:
        LogMsg(tr("A format error occurred while trying to write the data file."), Log::CRITICAL);
        break;
    default:
        LogMsg(tr("An unknown error occurred while trying to write the data file."), Log::CRITICAL);
        break;
    }
...
void GenericDataStorage::removeValue(const QString &key)
{
    const QWriteLocker locker {&m_lock};
...
bool GenericDataStorage::hasKey(const QString &key) const
{
    const QReadLocker locker {&m_lock};
