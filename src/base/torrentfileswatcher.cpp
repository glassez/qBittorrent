...
signals:
    void magnetFound(std::shared_ptr<BitTorrent::MagnetURI> magnetURI, const BitTorrent::AddTorrentParams &addTorrentParams);
    void torrentFound(std::shared_ptr<BitTorrent::TorrentFile> torrentFile, const BitTorrent::AddTorrentParams &addTorrentParams);
...
void TorrentFilesWatcher::onMagnetFound(const std::shared_ptr<BitTorrent::MagnetURI> magnetURI
        , const BitTorrent::AddTorrentParams &addTorrentParams)
{
    BitTorrent::Session::instance()->addTorrent(magnetURI, addTorrentParams);
}

void TorrentFilesWatcher::onTorrentFound(const std::shared_ptr<BitTorrent::TorrentFile> torrentFile
        , const BitTorrent::AddTorrentParams &addTorrentParams)
{
    BitTorrent::Session::instance()->addTorrent(torrentFile, addTorrentParams);
}
                                              ...
                                              , const TorrentFilesWatcher::WatchedFolderOptions &options)
{
    ...
    while (dirIter.hasNext())
    {
            ...
            if (file.open(QIODevice::ReadOnly | QIODevice::Text))
            {
                if (file.size() <= fileMaxSize)
                {
                    while (!file.atEnd())
                    {
                        ...
                        if (const auto parseResult = BitTorrent::MagnetURI::parse(line))
                            emit magnetFound(parseResult.value(), addTorrentParams);
                        else
        ...
        {
            if (const auto loadResult = BitTorrent::TorrentFile::loadFromFile(filePath))
            {
...
void TorrentFilesWatcher::Worker::processFailedTorrents()
{
    ...
    Algorithm::removeIf(m_failedTorrents, [this](const Path &watchedFolderPath, QHash<Path, int> &partialTorrents)
    {
        ...
        Algorithm::removeIf(partialTorrents, [this, &watchedFolderPath, &options](const Path &torrentPath, int &value)
        {
            ...
            if (const auto loadResult = BitTorrent::TorrentFile::loadFromFile(torrentPath))
            {
                BitTorrent::AddTorrentParams addTorrentParams = options.addTorrentParams;
